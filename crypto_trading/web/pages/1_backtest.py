...
with st.sidebar:
    ...
    market = st.selectbox("Market", ["futures", "spot"],
                          index=0 if settings.market_type == "futures" else 1)
...
if run_btn:
    if len(date_range) != 2:
        ...
        if not result.trades:
            ...
            c1.metric("Total Return", f"{ret_val:+.2f}%",
                      delta=f"{ret_val:+.2f}%", delta_color="normal")
            c2.metric("Sharpe Ratio", f"{metrics['sharpe_ratio']:.3f}")
            c3.metric("Max Drawdown", f"{metrics['max_drawdown_pct']:.2f}%",
                      delta=f"-{metrics['max_drawdown_pct']:.2f}%", delta_color="inverse")
            ...
            if result.equity_curve:
                ...
                fig.add_hline(y=float(capital), line_dash="dash",
